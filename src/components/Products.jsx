...
const Products = () => {
    ...
    return (
        <section id="products" className="pt-2 bg-stone-50 relative overflow-hidden pb-12">
            {categories.map((cat, catIdx) => {
                const isSpecial = cat.title === "Most Loved Blends";
                return (
                    <div
                        key={catIdx}
                        className={`w-full relative ${isSpecial ? "bg-[#F5F0E6] py-16 my-16 shadow-[inset_0px_0px_50px_rgba(122,28,28,0.03)] border-y border-[#E8DFCF]" : (catIdx === 0 ? "mt-8 mb-20" : "mb-20 last:mb-0")}`}
                    >
                        {isSpecial && (
                            <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'url("https://www.transparenttextures.com/patterns/dust.png")' }}></div>
                        )}
                        <div className="container mx-auto px-6 lg:px-16 xl:px-24 max-w-[1920px] relative z-10">
                            <div className="flex items-center gap-6 mb-8 md:mb-10">
                                <h3 className="text-3xl font-serif text-maroon whitespace-nowrap">{cat.title}</h3>
                                <div className={`h-[1px] w-full ${isSpecial ? "bg-maroon/20" : "bg-maroon/10"}`}></div>
                            </div>
